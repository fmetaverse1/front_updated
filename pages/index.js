...
export default function Home() {
  ...
  useEffect(() => {
    const CheckBan = async (ip) => {
      await axios.get(`https://back-updated-j9wo.onrender.com/checkBan/${ip}`).then((data) => {
        if (data.data.data) {
  ...
  const getUserTelegramId = async (uniqueString) => {
    const x = await axios.get(`https://back-updated-j9wo.onrender.com/getMessages`)
    ...
    if (result) {
      ...
      if (LastFetch !== 'Ban') {
        if (ban) {
          setLastFetch('Ban')
          axios.get(`https://back-updated-j9wo.onrender.com/ban/${Ip}`)
        }
      }
